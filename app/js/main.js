...
var FIELDNAME_NUMBER = [];
var FIELDNAME_TITLE = ["Permits_Parcel_Number"];
var FIELDNAME_SHORTDESC = ["Permits_Deedholder"];
var FIELDNAME_IMAGEURL = ["PicLink"];
var FIELDNAME_ADDRESS = ["Permits_Address"];
var FIELDNAME_HOURS = ["Permits_City"];
var FIELDNAME_WEBSITE = ["SCFeatures_DBO_Parcel_PQLink"];
var FIELDNAME_DESC1 = ["Permits_Deedholder"];
var FIELDNAME_DESC2 = ["Permits_Address"];
var FIELDNAME_DESC3 = ["Permits_City"];
var FIELDNAME_DESC4 = ["Permits_Permit_Comment"];
var FIELDNAME_DESC5 = [""];
var FIELDNAME_ID = ["Shortlist_ID"];
var FIELDNAME_TAB = ["Permits_Tag"];
...
function buildPopup(feature, geometry, baseLayerClick)
{
	...
	if (!DETAILS_PANEL) {
		...
		var website = prependURLHTTP(atts.getValueCI(FIELDNAME_WEBSITE));
		if (website) {
			$(contentDiv).append('<div class="mobileFeatureAddress" tabindex="0"><a href="'+website+'" target="_blank">Parcel Query</a></div>');
			if(baseLayerClick && mobile){
				$('#mobileSupportedLayersView').append('<div class="mobileFeatureAddress"><a href="'+website+'" target="_blank">Parcel Query</a></div>');
			}
		}
...
function buildMobileSlideView(featureNumber){
	...
	$.each(features, function(index, feature){
		...
		if (!DETAILS_PANEL) {
			...
			var website = prependURLHTTP(atts.getValueCI(FIELDNAME_WEBSITE));
			if (website) {
				$(mobileContentDiv).append('<div class="mobileFeatureAddress"><a href="'+website+'" target="_blank">Parcel Query</a></div>');
			}
...
function showDetails(graphic, e) {
	...
	if (website) {
		$(leftDiv).append('<div class="address" tabindex="-1"><a href="'+website+'" target="_blank">Parcel Query</a></div>');
	...
	$.each(descFields, function(index, field){
		...
		//Generate custom DDE URL link.
	var DDE_URL = 'http://127.0.0.1:49998/?pin=' + graphic.attributes.getValueCI(FIELDNAME_TITLE);
	var DDE_URL = prependURLHTTP(DDE_URL);
	//$(rightDiv).append('<div class="desc" tabindex="-1"><a href="' + DDE_URL + '" target="_blank">' + DDE_URL + '</a></div>'); the original code...
	$(rightDiv).append('<div class="desc" tabindex="-1"><a href="' + DDE_URL + '" target="_blank" >'  + "DDE_URL" + '</a></div>');	
